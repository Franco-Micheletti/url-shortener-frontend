...
export function ShortUrlResult () {
  const { shortUrl, handleSubmit, validationError, responseError } = useContext(ShortUrlContext)

  return (
    <div className='flex flex-col justify-center'>
      {
          shortUrl !== '' && !responseError && !validationError
            ? (
      ...
      <div className='flex justify-center flex-col'>
        ...
        <div className='flex justify-center'>
          <form className='flex justify-center w-2/3 flex-col gap-4' onSubmit={handleSubmit}>
            ...
            {
              responseError
                ? <div className='text-red-500 flex justify-center text-2xl rounded p-2'>{responseError}</div>
                : <></>
            }
            {
            validationError
